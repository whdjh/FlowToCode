function outputBlock(x, y, w, h, code){
    this.type = "outputBlock";
    this.x = x;
    this.y = y;
    this.w = w;
    this.h = h;
...
outputBlock.prototype = {
    ...
    render: function(){
        ...
        ctx.beginPath();
        // 크기 수정
        ctx.moveTo(this.x+this.w-10-6, this.y+this.h - 5); // -> 0, 5
        ctx.lineTo(this.x+this.w + 20-9, this.y+this.h - 5); // -> 20, 5
        ctx.lineTo(this.x+this.w + 5-6, this.y+this.h); // -> 10,0
        ctx.lineTo(this.x+this.w + 20-9, this.y+this.h-5); // -> 20, 5
        ctx.lineTo(this.x+this.w + 5-6, this.y+this.h -10); // -> 10, 10
        ctx.stroke();
